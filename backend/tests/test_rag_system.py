...
class TestRAGSystem(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if not cls.config.ANTHROPIC_API_KEY or cls.config.ANTHROPIC_API_KEY.startswith(
            "your-"
        ):
            cls.skipTest = True
            return
    ...
    def test_course_analytics(self):
        ...
        self.assertGreater(
            analytics["total_courses"], 0, "RAG system should have courses loaded"
        )
    ...
    def test_session_management(self):
        ...
        try:
            ...
            response1, sources1 = self.rag_system.query(
                "What is vector search?", session_id=session_id
            )
            ...
            response2, sources2 = self.rag_system.query(
                "How does it work?", session_id=session_id
            )
    ...
    def test_tool_execution_debugging(self):
        ...
        if "get_course_outline" in [
            t["name"] for t in self.rag_system.tool_manager.get_tool_definitions()
        ]:
            analytics = self.rag_system.get_course_analytics()
            if analytics["course_titles"]:
                course_title = analytics["course_titles"][0]
                outline_result = self.rag_system.tool_manager.execute_tool(
                    "get_course_outline", course_name=course_title
                )
    ...
    def test_vector_store_data_availability(self):
        ...
        print(
            f"Direct vector search results: {len(search_results.documents)} documents"
        )
