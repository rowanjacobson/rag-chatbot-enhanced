...
class ConversationState:
    ...
    def __init__(self, initial_query: str, conversation_history: Optional[str] = None, max_rounds: int = 2):
    ...
    def is_complete(self) -> bool:
        ...
        return (self.has_final_response or 
                self.error_occurred or 
    ...
    def should_continue_rounds(self) -> bool:
        ...
        return (not self.has_final_response and 
                not self.error_occurred and
                self.current_round < self.max_rounds and
                self.tools_this_round > 0)  # Only continue if tools were used
    
    def should_allow_tools(self) -> bool:
        """Determine if tools should be available for this round"""
        return (self.current_round < self.max_rounds and 
                self.total_tools_used < 5)  # Safety limit
...
"""
    ...
    def _execute_api_round(self, state: ConversationState, tools: Optional[List], tool_manager) -> Any:
        """Execute one round of API interaction"""
    ...
    def _process_response(self, response, state: ConversationState, tool_manager) -> bool:
        """
    ...
    def _handle_tool_round(self, response, state: ConversationState, tool_manager) -> bool:
        """
    ...
    def _handle_api_error(self, error: Exception, state: ConversationState) -> str:
        ...
        state.set_error("I encountered an error while processing your request. Please try again.")
        return state.get_final_response()
    ...
    def _generate_final_response_from_conversation(self, state: ConversationState) -> str:
                ...
                if isinstance(content, list):
                    for block in content:
                        if isinstance(block, dict) and block.get("type") == "tool_result":
                            ...
                            if result_content and not result_content.startswith("Tool execution failed"):
